...
export default class CollectionStorage {
  ...
  async downloadCategoryWords(categoryId) {
    const res = await fetch(`https://drakeman.cz/api/category/${categoryId}/words?shuffle=false`);
    var result = await res.json();
    if (result.payload === undefined) {
      return [];
    } else {
  ...
  async downloadCollectionWords(collectionId) {
    const res = await fetch(`https://drakeman.cz/api/collection/${collectionId}/words?shuffle=false`);
    var result = await res.json();
    if (result.payload === undefined) {
      return [];
    } else {
